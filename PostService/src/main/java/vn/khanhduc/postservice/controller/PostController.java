...
public class PostController {
    ...
    @GetMapping("/users")
    ResponseData<PageResponse<PostDetailResponse>> getAllPostsByUserLogin(
            @RequestParam(name = "page", required = false, defaultValue = "1") int page,
            @RequestParam(name = "size", required = false, defaultValue = "10") int size
    ) {
        return ResponseData.<PageResponse<PostDetailResponse>>builder()
                .code(HttpStatus.OK.value())
                .data(postService.getAllPostsByUserLogin(page, size))
                .build();
    }

    @DeleteMapping("/{id}")
    ResponseData<Void> deletePost(@PathVariable String id) {
